  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-3">
        <Clock size={13} className={featured ? 'text-black/55' : 'text-[#C8A45D]'} />
        <span className={`text-[11px] tracking-widest uppercase font-medium ${featured ? 'text-black/65' : 'text-[#A5A5A5]'}`}>
          {duration}
        </span>
      </div>
      <h3 className={`font-playfair text-2xl font-bold mb-2 leading-snug ${featured ? 'text-black' : 'text-white'}`}>{title}</h3>
      <p className={`text-4xl font-bold font-playfair mb-5 ${featured ? 'text-black' : 'text-[#C8A45D]'}`}>{price}</p>
      <p className={`text-sm leading-relaxed font-inter mb-8 flex-grow ${featured ? 'text-black/75' : 'text-[#A5A5A5]'}`}>{description}</p>
      <div className="flex items-center gap-2 mb-7">
        <CheckCircle size={13} className={featured ? 'text-black/55' : 'text-[#C8A45D]/55'} />
        <span className={`text-xs ${featured ? 'text-black/65' : 'text-[#A5A5A5]'}`}>No obligation</span>
      </div>
      <Button
